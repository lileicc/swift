...
<STR_LIT>\" {
            *string_buf_ptr = '\0';
            ...
            string s(string_buf);
            //yylval.sval = new BLOGSymbol<string>(curr_line, curr_col, s);
            yylval.sval = new BLOGSymbol<string>(curr_line, curr_col, str_buf);
            BEGIN(INITIAL);
...
<CHAR_LIT>\' {
            *string_buf_ptr = '\0';
            if (str_buf.length() == 1){
                ...
                yylval.chrval = new BLOGSymbol<char>(curr_line, curr_col, str_buf[0]);
            }
            else{
...
<STR_LIT,CHAR_LIT>\\b  { *string_buf_ptr++ = '\b'; buf_len++; str_buf.push_back('\b');}
<STR_LIT,CHAR_LIT>\\t  { *string_buf_ptr++ = '\t'; buf_len++; str_buf.push_back('\t');}
<STR_LIT,CHAR_LIT>\\n  { *string_buf_ptr++ = '\n'; buf_len++; str_buf.push_back('\n');}
<STR_LIT,CHAR_LIT>\\f  { *string_buf_ptr++ = '\f'; buf_len++; str_buf.push_back('\f');}
<STR_LIT,CHAR_LIT>\\r  { *string_buf_ptr++ = '\r'; buf_len++; str_buf.push_back('\r');}
<STR_LIT,CHAR_LIT>\\\" { *string_buf_ptr++ = '\"'; buf_len++; str_buf.push_back('\"');}
<STR_LIT,CHAR_LIT>\\\' { *string_buf_ptr++ = '\''; buf_len++; str_buf.push_back('\'');}
<STR_LIT,CHAR_LIT>\\\\ { *string_buf_ptr++ = '\\'; buf_len++; str_buf.push_back('\\');}
<CHAR_LIT,STR_LIT>. {
