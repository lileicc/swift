...
inline double uniformWeightLikely(const double*W, int n, int v) {
  if (v == 0) return log(W[0]);
  else return log(W[v] - W[v - 1]);
}
...
int get_first() {
  if (mark_first == cur_loop) return ret_first;
  mark_first = cur_loop;
  int&ret = ret_first;
...
int get_damage(int);
int get_prep(int i) {
  if (mark_prep[i] == cur_loop) return ret_prep[i];
...
int get_damage(int i) {
  if (mark_damage[i] == cur_loop) return ret_damage[i];
